...
namespace RolexDevStudio
{
    ...
    public static class VSUtility
    {
        ...
        public static bool HasClassOrStruct(System.Collections.IEnumerable projectItems,string @namespace, string name)
        {
            foreach (var c in GetProjectItems(projectItems))
            {
                ...
                foreach (var ele in eles.CodeElements)
                {
                    if (ele is EnvDTE.CodeNamespace)
                    {
                        ...
                        if (ns.Name.Equals(@namespace, StringComparison.InvariantCulture))
                        {
                            // run through classes
                            foreach (var t in ns.Members)
                            {
                                var type = t as CodeType;
                                if (null!=type && type.Kind == vsCMElement.vsCMElementClass || type.Kind == vsCMElement.vsCMElementStruct)
                                {
        ...
        public static IEnumerable<ProjectItem> GetProjectItems(System.Collections.IEnumerable projectItems)
        {
